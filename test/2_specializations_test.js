...
contract('Specializations', function(accounts) {
  it("should return generated code for init specializations correctly", function() {
    var contract;
    return Specializations.deployed().then(instance => {
      contract = instance;
      var promises = [];
    ...
    .then(() => contract.generateCode(specialization, skill))
    .then(data => assert.equal(data.toString(), code, "'mobile' and 'android' code wasn't get correct"));
  });

  it("should return specialization and skill by given code correctly", function() {

    return Specializations.deployed().then(instance => instance.getSpecializationByCode(code))
    .then(data => {
